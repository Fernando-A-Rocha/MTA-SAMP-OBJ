...
function loadTextureStudioMap(mapid,parsed,int,dim)
    ...
    for _, v in pairs(parsed) do
        ...
        if v.f == "model" then
            ...
            local allocated_id = lib:AddSimpleModel(baseid, newid, path, dffname, txdname, colname)
            if allocated_id then
                table.insert(loaded_maps[mapid].models, allocated_id)
            end
        elseif v.f == "object" then
            local obj,lod = lib:CreateNewObject(unpack(v.variables))
            last_object = obj
        ...
        elseif v.f == "material" then
            if isElement(last_object) then
                ...
                local elements = lib:SetObjectMaterial(last_object, mat_index,model_id,tex_name,color)
...
function doLoadSAMPObjects(objs, mapid)
    ...
    if lib:queueSAMPIDs(objs, mapid)
    and lib:loadSAMPObjects() then
